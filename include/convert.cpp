...
mapper_t design2genome(Design* design, representation::representation *repres) {
	...
	for (auto mod : design->selected_modules()) {
		...
		for (auto output : mapper.out) {
			...
			info_message("[INFO] load: deleting output wire %u - %s from chromosome\n", output.first, output.second.wire->name.c_str());
		...
		for (auto &it : mod->wires_) {
			...
			if (!wire->port_output && !wire->port_input) { //is inner
				info_message("[INFO] load: deleting inner wire %s\n", wire->name.c_str());
	...
	info_message("[INFO] load: loaded chromosome with %u gens, %lu inputs and %lu outputs\n", repres->chromosome->size(), mapper.in.size(), mapper.out.size());
